...
export function resolveAgentTarget(name: string, itemId?: string, scopeId?: string): Element | undefined {
  ...
  if (scopeId) {
    const scope = document.querySelector(`[data-embinder-scope="${esc(scopeId)}"]`);
  ...
  const byFocus = document.querySelectorAll(`[data-embinder-focus-for~="${esc(name)}"]`);
  if (byFocus.length === 1) return byFocus[0];
  if (byFocus.length > 1) return largestVisible(byFocus) ?? byFocus[0];
